...
class StationRegistry:
    ...
    def get_buoys_by_source(self, source: str) -> Dict:
        ...
        return {bid: data for bid, data in self.buoys.items() if data.get("source") == source}

    def get_tide_stations(
        self, has_observations: bool = None, has_predictions: bool = None, station_type: str = None
    ) -> Dict:
        """
    ...
    def get_all_coordinates(self) -> List[Dict]:
        ...
        for bid, data in self.buoys.items():
            coords.append({"type": "buoy", "id": bid, "name": data["name"], "lat": data["lat"], "lon": data["lon"]})

        for key, data in self.tides.items():
            coords.append({"type": "tide", "id": key, "name": data["name"], "lat": data["lat"], "lon": data["lon"]})
