...
**Current Server:** Dell Latitude 7480 laptop (as of February 2026)
- **Previous hardware:** Lenovo M910q (until Feb 1, 2026 - hardware failure)
- **Migration:** Feb 1-8, 2026 (4 hours downtime)
- **OS:** Ubuntu Server 24.04 LTS
- **Platform:** Linux 6.8.0-100-generic

**Storage:**
- **Internal:** 238.5GB Samsung NVMe SSD (system drive)
- **External:** 223.6GB USB SATA drive (webcam archives & backups)
  - Device: `/dev/sda1`
...
**Network:**
- Web server: Caddy on port 8090
- Domain: halibutbank.ca
...
# Frontend output: ~/envcan_wave/site/data/stations.json
...
# The 13:31 run fetches the 00Z forecast and archives it for verification
...
# Daily 02:13 UTC: export forecast verification (predictions issued 56-79 h ahead)
0 14 * * * cd /home/keelando/envcan_wave && source .venv/bin/activate && python3 export_storm_surge_verification.py >> ~/envcan_wave/storm_surge_verification.log 2>&1
```

### Webcam Capture

See **[WEBCAM_PIPELINE.md](WEBCAM_PIPELINE.md)** for complete documentation.
...
# Cox Bay: Every 15 minutes (daylight only)
...
# Mud Bay HD: Every 30 minutes (daylight only)
...
# Ambleside (Hollyburn Sailing Club): Every 20 minutes (daylight only)
...
**Archive storage:** `/mnt/storage/[webcam]_cam/` (external USB drive)

**Website output:** `~/envcan_wave/site/data/[webcam]/` (latest.jpg, slideshow/, metadata)
...
The canonical crontab is `config/crontab.txt` (tracked in git). **Do not use `crontab -e` directly** — edits made that way will be overwritten by the nightly auto-backup.

**To change a cron job:**
```bash
# 1. Edit the canonical file
$EDITOR config/crontab.txt

# 2. Preview the diff against the live crontab
scripts/install_crontab.sh --diff-only

# 3. Apply (snapshots live crontab to ~/crontab_backups/ first, validates all script paths)
scripts/install_crontab.sh
```

Use `--force` to install even if a referenced script path is missing.
...
:8090 {
    ...
    # HTTP access logging in JSON format
    log {
        output file /var/log/caddy/halibutbank-access.log
    }

    # Block scanner probes for sensitive paths
    @scanners {
        path /.git/* /.env /.env.* /wp-admin/* /wp-login.php /xmlrpc.php /phpmyadmin/*
    }
    ...
    header {
        ...
        Content-Security-Policy "default-src 'self'; script-src 'self' https://unpkg.com https://cdnjs.cloudflare.com https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://unpkg.com; img-src 'self' data: https:; font-src 'self'; connect-src 'self'; frame-ancestors 'none'"
    ...
    # NO CACHE for webcam images (updated every 10 minutes)
    @webcam {
        path /data/wrcam/* /data/bbcam/*
    }
    header @webcam Cache-Control "no-store, no-cache, must-revalidate"

    # Cache other images for 1 month (includes banner, static images)
    @images {
        path *.jpg *.jpeg *.png *.gif *.webp *.svg
        not path /data/wrcam/* /data/bbcam/*
    }
...
```bash
cd ~/envcan_wave/site/assets/css
...
# Edit ~/envcan_wave/site/index.html, tides.html, forecasts.html
...
chmod 644 ~/envcan_wave/site/data/*.json

# Note: stations.json is automatically synced by export_stations_json.py (runs hourly)
# Backend source: ~/envcan_wave/config/stations.json
# Frontend copy: ~/envcan_wave/site/data/stations.json (auto-updated, do not edit manually)
...
ls -lh ~/envcan_wave/site/data/latest_buoy_v2.json ~/envcan_wave/site/data/tide-latest.json ~/envcan_wave/site/data/marine_forecast.json
```
...
**Restore crontab:**
```bash
# Install from canonical file (validates script paths, snapshots live first)
scripts/install_crontab.sh

# OR restore directly from a snapshot
crontab ~/crontab_backups/<snapshot>.txt
...
1. **Clone repositories:**
   ...
   git clone https://github.com/yourusername/site.git ~/envcan_wave/site
   ```
...
```bash
# Fix ownership
sudo chown -R keelando:keelando ~/envcan_wave ~/envcan_wave/site

# Fix permissions
chmod 755 ~/envcan_wave ~/envcan_wave/site
chmod 644 ~/envcan_wave/site/data/*.json
chmod 600 ~/.config/buoy_influx_1.env
```
