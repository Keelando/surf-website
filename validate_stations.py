...
def validate_buoy(buoy_id: str, data: Dict, all_ids: Set, all_names: Set) -> List[str]:
    ...
    if data['source'] not in VALID_BUOY_SOURCES:
        warnings.append(
            f"Buoy {buoy_id}: Unknown source '{data['source']}' "
            f"(expected: {', '.join(VALID_BUOY_SOURCES)})"
        )
...
def validate_tide(station_key: str, data: Dict, all_ids: Set, all_names: Set, all_codes: Set) -> List[str]:
    ...
    if data['type'] not in VALID_TIDE_TYPES:
        warnings.append(
            f"Tide station {station_key}: Invalid type '{data['type']}' "
            f"(expected: {', '.join(VALID_TIDE_TYPES)})"
        )
    ...
    if data['type'] == 'TEMPORARY' and 'wlo' in data.get('series', []):
        warnings.append(
            f"Tide station {station_key}: TEMPORARY stations should not have 'wlo' series"
        )
    ...
    if 'update_frequency_minutes' in data:
        ...
        if has_obs and freq is None:
            warnings.append(
                f"Tide station {station_key}: Has observations but update_frequency_minutes is null"
            )
        elif not has_obs and freq is not None:
            warnings.append(
                f"Tide station {station_key}: No observations but update_frequency_minutes is set"
            )
...
def main():
    ...
    try:
        ...
        for station_key, station_data in tides.items():
            try:
                warnings = validate_tide(
                    station_key, station_data, 
                    all_tide_ids, all_tide_codes, all_tide_names
                )
                all_warnings.extend(warnings)
            except ValidationError as e:
                print(f"  ❌ {e}")
